...
class PuddyWeb3 {
  ...
  // Data
  getBlockchain() {
    return clone(this.networks[this.network]);
  }
  isEnabled() {
    return this.enabled;
  }
  ...
  async testMessage() {
    if (this.enabled) {
      ...
      const message = ethers.utils.toUtf8Bytes(
        'This is a tiny Pudding Web3 Test! :3' + '\n\nNonce: 0',
      );
        ...
        try {
          ...
          if (contract_address) {
            ...
            if (typeof contract_address === 'string') {
              contract_address = { value: contract_address, decimals: 18 };
            }
            if (typeof contract_address.value !== 'string') {
              contract_address.value = '';
            }
            if (typeof contract_address.decimals !== 'number') {
            ...
            const valueToTransfer = base
              .pow(contract_address.decimals)
              .times(String(send_token_amount));

            // Transaction
            transaction = await tinyThis.executeContract(
              contract_address.value,
              {
                type: 'function',
                name: 'transfer',
      ...
      if (tinyThis.enabled) {
        ...
        try {
          ...
          if (tinyThis.address) {
            ...
            const signature = await tinyThis.provider.send('personal_sign', [
              ethers.utils.hexlify(msg),
              address,
            ]);
  ...
  async requestAccounts(changeNetwork = true) {
    ...
    if (
      ...
      Number(this.provider.network.chainId) !== Number(this.networks[this.network].chainIdInt)
    ) {
