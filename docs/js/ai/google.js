const setGoogleAi = (tinyGoogleAI, GEMINI_API_KEY, MODEL_DATA = {
  id: 'gemini-1.5-flash',
  ...
  const buildErrorData = (result, finalData) => {
    ...
    if (result.error.details)
      finalData.error.details = result.error.details;
  };
  ...
  tinyGoogleAI._setGenContent((apiKey, selectedModel, isStream, data) => new Promise((resolve, reject) =>
    fetch(`${apiUrl}/models/${selectedModel}:${!isStream ? 'generateContent' : 'streamGenerateContent'}?key=${encodeURIComponent(apiKey)}`, {
      method: 'POST',
      headers: {
      ...
      .then(res => res.json()).then((result) => {
        ...
        if (!result.error) {
          ...
          // Model Version
          finalData.modelVersion = typeof result.modelVersion === 'string' ?
          ...
          if (result.usageMetadata) {
            ...
            if (typeof result.usageMetadata.candidatesTokenCount === 'number')
              finalData.tokenUsage.count.candidates = result.usageMetadata.candidatesTokenCount;
            else
              needShowMetadataError = true;

            // Prompt
            if (typeof result.usageMetadata.promptTokenCount === 'number')
              finalData.tokenUsage.count.prompt = result.usageMetadata.promptTokenCount;
            else
              needShowMetadataError = true;

            // Total
            if (typeof result.usageMetadata.totalTokenCount === 'number')
              finalData.tokenUsage.count.total = result.usageMetadata.totalTokenCount;
            else
              needShowMetadataError = true;
          }
          // Error
  ...
  tinyGoogleAI._setInsertServerCache((apiKey, modelName, cacheName, data) => new Promise((resolve, reject) =>
    fetch(`${apiUrl}/cachedContents?key=${encodeURIComponent(apiKey)}`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(requestBuilder(data, { model: modelName, name: cacheName })),
    })
      // Request
      .then(res => res.json()).then((result) => {
        // Prepare final data
  ...
  tinyGoogleAI._setGetServerCache((apiKey, cacheName) => new Promise((resolve, reject) =>
  ...
  tinyGoogleAI._setGetModels((apiKey, pageSize, pageToken) => new Promise((resolve, reject) =>
    fetch(`${apiUrl}/models?key=${encodeURIComponent(apiKey)}&pageSize=${encodeURIComponent(pageSize)}${pageToken ? `&pageToken=${encodeURIComponent(pageToken)}` : ''}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
        ...
        if (!result.error) {
          ...
          for (const index in result.models) {
            const newModel = {
              ...
              supportedGenerationMethods: result.models[index].supportedGenerationMethods,
            };
            ...
            if (inserted)
              finalData.newData.push(inserted);
          }
